...
// Los 4 modelos estan verificados contra /api/health?models=1 con esta key.
// gemini-2.0-flash estaba en la cadena y la key NO tiene acceso: era el ultimo
// eslabon, asi que cuando los tres primeros fallaban el fallback terminaba en un
// 403 PERMISSION_DENIED garantizado. Un cliente subio una cartola real y eso fue
// lo que vio, en ingles y con codigo de error.
export const MODEL_CHAIN = [MODEL, "gemini-flash-latest", "gemini-3.5-flash", "gemini-2.5-flash"];

// Vale la pena pasar al siguiente modelo tanto si el fallo es transitorio
// (sobrecarga, cuota) como si es de disponibilidad (la key no puede usar ESE
// modelo). Antes un 403/404 tiraba la request completa aunque quedaran modelos
// buenos mas abajo en la cadena.
function pasarAlSiguienteModelo(err) {
  const s = String((err && err.message) || err);
  return /50[0-9]|429|UNAVAILABLE|RESOURCE_EXHAUSTED|high demand|overloaded/i.test(s)
    || /\b40[34]\b|PERMISSION_DENIED|NOT_FOUND|does not have permission|is not found/i.test(s);
}

// Nunca mostrarle al usuario el error crudo de la API: llega en ingles y con
// codigos. Se traduce a algo accionable y el detalle queda en el log.
export function mensajeUsuario(err) {
  const s = String((err && err.message) || err);
  if (/PERMISSION_DENIED|\b403\b|does not have permission/i.test(s)) {
    return "No pudimos leer tu documento con el motor de IA. Prueba con una foto del documento en vez del PDF, o escríbenos por WhatsApp y lo procesamos a mano.";
  }
  if (/RESOURCE_EXHAUSTED|429|quota/i.test(s)) {
    return "Estamos con mucha demanda en este momento. Intenta de nuevo en un par de minutos.";
  }
  if (/50[0-9]|UNAVAILABLE|overloaded/i.test(s)) {
    return "El servicio de IA no respondió a tiempo. Intenta de nuevo en un momento.";
  }
  if (/timeout|abort/i.test(s)) {
    return "Tu documento tardó demasiado en procesarse. Prueba con un archivo más liviano o con una foto.";
  }
  if (/JSON|parse|Respuesta vacía|no contiene/i.test(s)) {
    return "La respuesta llegó incompleta. Intenta de nuevo: suele funcionar al segundo intento.";
  }
  return "No pudimos generar tu resultado. Intenta de nuevo en un momento.";
}
...
export async function generateConFallback(ai, req) {
  ...
  for (const m of MODEL_CHAIN) {
    try {
      ...
      ultimo = e;
      if (!pasarAlSiguienteModelo(e)) throw e;
