...
const RUBRICA = `Eres el agente de QA de Ruta PAES. Revisa CADA ejercicio contra esta rubrica:
1. FORMATO: enunciado claro y autocontenido, exactamente 4 alternativas A-D, una sola correcta.
2. TEMARIO: el contenido corresponde al eje declarado y al temario oficial DEMRE de esa prueba (nivel escolar chileno, no universitario).
...
4. LENGUAJE: espanol de Chile claro, sin ambiguedades, sin copiar items reales de DEMRE.
...
function extractJson(text) {
  if (!text) throw new Error("QA: respuesta vacia");
  let t = text.trim();
  const fence = t.match(/```(?:json)?\s*([\s\S]*?)```/);
  if (fence) t = fence[1].trim();
  const start = t.indexOf("{");
  const end = t.lastIndexOf("}");
  if (start === -1 || end === -1) throw new Error("QA: sin JSON");
 ...
 */
export async function qaDrills(drills, prueba) {
